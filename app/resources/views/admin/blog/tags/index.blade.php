                ...
                <div class="stack" style="gap:10px;">
                    @foreach($tags as $item)
                        <form method="POST" action="{{ route('admin.blog.tags.update', $item) }}" class="card" style="padding:12px; display:grid; gap:10px;">
                            ...
                            <div class="form-grid">
                                <div class="field">
                                    <label for="tag_name_{{ $item->id }}">
                                        Nome
                                        @include('partials.help-hint', ['text' => 'Nome exibido da tag para cruzar artigos por assunto.'])
                                    </label>
                                    <input id="tag_name_{{ $item->id }}" class="input" name="name" value="{{ old('name.'.$item->id, $item->name) }}" required>
                                </div>

                                <div class="field">
                                    <label for="tag_slug_{{ $item->id }}">
                                        Slug
                                        @include('partials.help-hint', ['text' => 'Parte da URL da tag. Ideal para SEO interno e organização semântica.'])
                                    </label>
                                    <input id="tag_slug_{{ $item->id }}" class="input mono" name="slug" value="{{ old('slug.'.$item->id, $item->slug) }}">
                                </div>

                                <div class="field full">
                                    <label for="tag_desc_{{ $item->id }}">
                                        Descrição
                                        @include('partials.help-hint', ['text' => 'Contexto da tag para uso editorial e padronização da equipe.'])
                                    </label>
                                ...
                                <label class="radio-card" for="tag_featured_{{ $item->id }}" style="margin-top: 22px;">
                                    ...
                                    <span>
                                        Tag destacada
                                        @include('partials.help-hint', ['text' => 'Tags destacadas podem ganhar maior visibilidade nos blocos laterais da vitrine.'])
                                    </span>
                                </label>
                            </div>
